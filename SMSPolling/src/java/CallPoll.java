...
public class CallPoll extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (phnNo.startsWith("0"))
                phnNo=phnNo.substring(1);
            ResultSet livePollRS = ManageConnection.getDBConnection().createStatement().executeQuery("SELECT * FROM polls WHERE active = 1 AND phnNo LIKE '%"+phnNo+"%'");
